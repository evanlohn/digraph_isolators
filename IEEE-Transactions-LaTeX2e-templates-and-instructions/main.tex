...
\begin{enumerate}
\item A ``variable'' is an object that can be assigned to either True or False 
\item A positive or negative ``literal'' is a shorthand for the predicate that a given variable is assigned True or False, respectively. 
\item A ``clause'' is a disjunction of 0 or more literals.
\item A ``unit clause'' (sometimes referred to as simply a ``unit'') is a clause containing exactly one literal.
\item A ``CNF formula'' is a conjunction of clauses (CNF=Conjunctive Normal Form). Unless otherwise specified, ``formula'' refers to ``CNF formula.''
\item An assignment $\sigma$ is a function from variables to truth values (True/False). $\sigma$ satisfies a formula $F$ if the boolean function denoted by $F$ returns True given the inputs specified by $\sigma$.
\end{enumerate}
...
\item Two $n$-vertex tournaments $G_1, G_2$ are isomorphic (written $G_1 \simeq G_2$ exactly when there exists a permutation $p$ on the vertices of $G_1$ such that $p(G_1) = G_2$. When any such $p$ exists, it is referred to as an isomorphism between $G_1$ and $G_2$.
\item The ``equivalence class'' (also, ``isomorphism class'') $E_G$ of a tournament $G$ is defined as $E_G = \{G' | G \simeq G'\}$.
\item An automorphism $p$ on a tournament $G$ is any $p$ such that $p(G) = G$. The set of automorphisms of $G$ form a group under function composition; this group is referred to as $Aut(G)$.
\end{enumerate}

Finally, we define the following isolator-related concepts:
\begin{enumerate}
\item Some or all of the variables in a formula $F$ can be defined to have the semantics ``edge $e$ exists in graph $G$''. If this is the case, we say that $F$ admits $G$ exactly when there exists a satisfying assignment to the conjunction of $F$ and the set of unit clauses semantically implied by the edges of $G$.
...
\item A perfect isolator is an isolator that admits \textit{exactly} one tournament from each equivalence class.
\item A perfect isolator $F$ is ``optimal'' if there does not exist a perfect isolator with fewer non-unit clauses than $F$.
\item A ``compact'' or ``short'' isolator is not rigorously defined; rather it describes an isolator with few enough non-unit clauses to be of practical use in solving SAT problems.
\end{enumerate}
%TBD
...
\begin{equation}
Kills(G,c) = \bigwedge\limits_{a \in A_G}\lnot In(c, a)
\end{equation}
...
Finally, we considered whether to count these special unit literals towards the clause count in determining isolator optimality. As mentioned in the preliminaries, we chose not to do so. When an isolator with units is used in a SAT solver, the units will be instantly eliminated through unit propagation and thus will reduce the complexity of the resulting problem.  Therefore, we consider an `optimal isolator' to not just have the minimal number of clauses, but the minimal number of non-unit clauses.  Since units cannot exist in undirected graph isolators, this definition of optimality is consistent with the prior work on the undirected case \cite{ref_heule}. Note that we only needed to consider positive unit literals as per the vertex-labeling symmetry breaking, which drastically reduced the search space.
